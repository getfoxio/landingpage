const getfox = {
  ...
  init: (options) => {
    return new Promise((resolve) => {
      getfox.key = options.key
      getfox.username = ''
      if (!!options.username) {
        getfox.username = options.username
      }
      getfox.apiUrl = 'https://api.getfox.io/api'
      if (options.apiUrl) {
        getfox.apiUrl = options.apiUrl
      }
      getfox.build()
      resolve()
    })
  },
  user: (username = getfox.username, key = getfox.key) => {
    const query = `{
  ...
  userlinks: (username = getfox.username, key = getfox.key) => {
    const query = `{
  ...
  fetchApi: (query) => {
    ...
    return fetch(getfox.apiUrl, options).then((res) => res.json())
  },
  build: async () => {
    try {
      user = await getfox.user()
      if (user) {
        getfox.executeBuild(user)
      } else {
      }
    } catch (err) {
      console.error(err)
    ...
    html = `
      ...
      <div class="profile-landingpage">
        ...
        ${
          !user.links.length
            ? `
          <div class="fc">No links found.</div>
        `
            : `${user.links
                .map((link) => {
                  let product = ''
                ...
                <div class="flc product">
                  <div>${
                    link.product.price
                  } ${link.product.currency.toUpperCase()}</div>
                </div>`
                  }
                  let linkImageSource = ''
                  if (!!link.background) {
